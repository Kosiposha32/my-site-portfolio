...
const observer = new IntersectionObserver(
  ...
  const horizontalProjects = projects.filter((project) => project.category !== 'Reels');
  const reelsProjects = projects.filter((project) => project.category === 'Reels');
            ...
            {horizontalProjects.map((project) => (
              ...
              >
                <div className={cn(
                  ...
                  theme === 'dark' ? 'bg-[#111118]' : 'bg-slate-100'
                )}>
                  {project.embedUrl ? (
                    <iframe
                ...
                <div className="p-5 xl:p-6">
                  <h3 className={cn(
                    "text-lg xl:text-xl font-bold mb-2 transition-colors",
                    theme === 'dark' ? 'text-slate-100 group-hover:text-rose-300' : 'text-slate-900 group-hover:text-rose-600'
                  )}>
                    {project.title}
                  </h3>
                  <p className={cn('text-sm xl:text-base leading-relaxed', theme === 'dark' ? 'text-slate-400' : 'text-slate-600')}>
                    {project.description}
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 xl:gap-8">
              {reelsProjects.map((project) => (
                <div 
                  key={project.id}
                  className={cn(
                    "group w-full max-w-[280px] sm:max-w-[300px] rounded-3xl overflow-hidden transition-all hover:-translate-y-2",
                    theme === 'dark'
                      ? 'bg-[#1b1b25] border border-white/5 shadow-lg shadow-black/30 hover:shadow-black/45'
                      : 'bg-white shadow-lg shadow-slate-200/50 hover:shadow-xl hover:shadow-slate-300/50'
                  )}
                >
                  <div className={cn(
                    "relative mx-auto overflow-hidden aspect-[9/16] max-h-[520px] w-full",
                    theme === 'dark' ? 'bg-[#111118]' : 'bg-slate-100'
                  )}>
                    {project.embedUrl ? (
                      <iframe
